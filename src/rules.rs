...
pub type Generation = Vec<Vec<bool>>;

//type rule_kernel = Vec<Vec<bool>>;

//const rule_kernel: [[], ];
pub const CELL_IS_ALIVE: bool = true;
pub const CELL_IS_DEAD: bool = false;
...
impl Universe {
    ...
    // fn is_need_to_be_killed(current_cell, number_of_alive_neighbouts) -> bool
    // {
    //     return current_cell_is_alive && (number_of_cell_neighbours < 2 || number_of_cell_neighbours > 3));
    // }

    // fn is_need_to_be_alived(current_cell, number_of_alive_neighbouts) -> bool
    // {
    ...
    //     let neighbours = convolve_with_rule_kernel(previous_generation);
        
    //     for (each_cell)
    //     {
    //         if (is_need_to_be_alived(current_cell, neighbours[]))
    //         {
